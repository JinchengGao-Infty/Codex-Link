...
async fn apply_explorer_role_preserves_current_model_and_reasoning_effort() {
    ...
    // The explorer role layer installs developer instructions but must not
    // displace the caller's sticky model or reasoning-effort choices.
    ...
    assert!(
        config
            ...
            .as_deref()
            .is_some_and(|instructions| instructions.contains("read-only reconnaissance agent")),
        "explorer role should install its developer instructions"
    );
}
